...
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'uint8_strlen'
;------------------------------------------------------------
;str                       Allocated with name '_uint8_strlen_str_65536_131'
;length                    Allocated with name '_uint8_strlen_length_65536_132'
;------------------------------------------------------------
;	report.c:6: size_t uint8_strlen(const uint8_t *str) {
;	-----------------------------------------
;	 function uint8_strlen
;	-----------------------------------------
_uint8_strlen:
	ar7 = 0x07
	ar6 = 0x06
	...
	mov	r6,dph
	mov	a,dpl
	mov	dptr,#_uint8_strlen_str_65536_131
	movx	@dptr,a
	mov	a,r6
	inc	dptr
	movx	@dptr,a
	...
	inc	dptr
	movx	@dptr,a
;	report.c:8: while (str[length] != '\0') { // Check for the null terminator
	mov	dptr,#_uint8_strlen_str_65536_131
	movx	a,@dptr
	mov	r5,a
	inc	dptr
...
;	report.c:9: length++;
	...
	sjmp	00101$
00103$:
;	report.c:11: return length;
	mov	dpl,r3
	mov	dph,r4
;	report.c:12: }
	ret
;------------------------------------------------------------
;Allocation info for local variables in function 'report_init'
;------------------------------------------------------------
;	report.c:14: void report_init(void) {
;	-----------------------------------------
;	 function report_init
;	-----------------------------------------
_report_init:
;	report.c:16: gpioConfigure(&led_pin);
	mov	dptr,#_led_pin
	mov	b,#0x00
	lcall	_gpioConfigure
;	report.c:17: led_timer_count = get_current_time();
	lcall	_get_current_time
	mov	r4,dpl
	mov	r5,dph
	mov	r6,b
	mov	r7,a
	mov	dptr,#_led_timer_count
	mov	a,r4
	movx	@dptr,a
	mov	a,r5
	inc	dptr
...
;	report.c:19: }
	ret
;------------------------------------------------------------
;Allocation info for local variables in function 'report_toggle_led'
;------------------------------------------------------------
;__1966080008              Allocated with name '_report_toggle_led___1966080008_196608_139'
;config                    Allocated with name '_report_toggle_led_config_262144_140'
;------------------------------------------------------------
;	report.c:21: void report_toggle_led(void) {
;	-----------------------------------------
;	 function report_toggle_led
;	-----------------------------------------
_report_toggle_led:
;	report.c:22: if ((get_current_time() - led_timer_count) >= LED_BLINK_PERIOD) {
	lcall	_get_current_time
	mov	r4,dpl
	mov	r5,dph
	mov	r6,b
	...
	subb	a,#0x00
	jc	00104$
;	report.c:24: gpioToggle(&led_pin);
...
00111$:
	...
	mov	b,#0x00
	lcall	_gpioWrite
;	report.c:25: led_timer_count = get_current_time();                          
	lcall	_get_current_time
	mov	r4,dpl
	mov	r5,dph
	mov	r6,b
...
;	report.c:28: }
	ret
;------------------------------------------------------------
;Allocation info for local variables in function 'report'
;------------------------------------------------------------
;__fmt                     Allocated to stack - _bp -5
;vaList                    Allocated to registers r7 
;------------------------------------------------------------
;	report.c:31: void report(const char* __fmt, ...) {
;	-----------------------------------------
;	 function report
;	-----------------------------------------
_report:
	push	_bp
;	report.c:35: va_start(vaList, __fmt);
	mov	a,sp
	mov	_bp,a
	add	a,#0xfb
	mov	r7,a
;	report.c:36: vsprintf(_buffer, __fmt, vaList);
	mov	a,_bp
	add	a,#0xfb
	mov	r0,a
	...
	mov	b,#0x00
	lcall	_vsprintf
;	report.c:40: nrf24_device(TRANSMITTER, RESET);
	mov	dptr,#_nrf24_device_PARM_2
	mov	a,#0x01
	movx	@dptr,a
	mov	dpl,#0x00
	lcall	_nrf24_device
;	report.c:41: nrf24_CE(0);
	mov	dpl,#0x00
	lcall	_nrf24_CE
;	report.c:43: while(nrf24_transmit("testtesttesttest", 16, ACK_MODE) == TRANSMIT_FAIL) { printf("nrf24 failed to send!"); }
00101$:
...
;	report.c:52: printf("\n");
	...
	dec	sp
	dec	sp
;	report.c:53: nrf24_device(RECEIVER, RESET);
	mov	dptr,#_nrf24_device_PARM_2
	mov	a,#0x01
	movx	@dptr,a
	mov	dpl,#0x01
	lcall	_nrf24_device
;	report.c:56: printf(_buffer);
	mov	a,#__buffer
	push	acc
	mov	a,#(__buffer >> 8)
	push	acc
...
;	report.c:57: } 
	...
	.area CONST   (CODE)
___str_0:
	.ascii "testtesttesttest"
	.db 0x00
	.area CSEG    (CODE)
	.area CONST   (CODE)
___str_1:
	.ascii "nrf24 failed to send!"
	.db 0x00
	.area CSEG    (CODE)
	.area CONST   (CODE)
___str_2:
