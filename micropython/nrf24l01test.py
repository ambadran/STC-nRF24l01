...
spi = SPI(0, baudrate=100_000, sck=Pin(18), mosi=Pin(19), miso=Pin(16))
cfg = {"spi": spi, "csn": 20, "ce": 21}


# Addresses are in little-endian format. They correspond to big-endian
# 0xf0f0f0f0e1, 0xf0f0f0f0d2
# pipes = (b"\xf0\xf0\xf0\xf0\xf0", b"\xf0\xf0\xf0\xf0\xf0")
...
def initiator():
    ...
    while num_successes < num_needed and num_failures < num_needed:
        ...
        try:
            ...
            # msgs = 'ckdiek'
            # msg = msgs[0]
            nrf.send(struct.pack("s", bytearray('c'.encode())))
        except OSError:
            pass
...
def responder():
    ...
    spi = cfg["spi"]
    global nrf
    nrf = NRF24L01(spi, csn, ce, payload_size=16)  # edited from 8 to 32
    ...
    while True:
        if nrf.any():
            while nrf.any():
                ...
                # millis = struct.unpack('s', buf)
                # millis = millis[0].decode()
                # led_state = struct.unpack('s', buf)
                # led_state = millis[0].decode()
...
def print_registers():
    global nrf
    for i in range(24):
        print(f"Register {i}: {nrf.reg_read(i)}")
        utime.sleep_ms(200)
